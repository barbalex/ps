...
export const usePlaceActionReportNavData = ({
  projectId,
  subprojectId,
  placeId,
  placeId2,
  ...
  const sql = `
      WITH
        place_action_report_quantities_count AS (SELECT count(*) FROM place_action_report_quantities WHERE place_action_report_id = '${placeActionReportId}')
      SELECT
      ...
      WHERE 
        place_action_reports.place_action_report_id = '${placeActionReportId}'`
  ...
  const resPlaceLevel = useLiveQuery(
    `SELECT place_action_report_quantities, place_action_report_quantities_in_report FROM place_levels WHERE project_id = $1 AND level = $2`,
  ...
  const navData = {
    ...
    notFound,
    navs:
      placeLevel?.place_action_report_quantities_in_report === false
        ? [
            {
              id: 'report',
