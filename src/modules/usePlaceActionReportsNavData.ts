...
export const usePlaceActionReportsNavData = ({
  projectId,
  subprojectId,
  placeId,
  placeId2,
  ...
  const ownArray = [...parentArray, 'action-reports']
  ...
  const sql = isOpen
    ? `
      WITH
        count_unfiltered AS (SELECT count(*) FROM place_action_reports WHERE place_id = '${placeId2 ?? placeId}'),
        count_filtered AS (SELECT count(*) FROM place_action_reports WHERE place_id = '${placeId2 ?? placeId}' ${isFiltered ? ` AND ${filterString}` : ''})
      SELECT
        place_action_report_id AS id,
        label,
        count_unfiltered.count AS count_unfiltered,
        count_filtered.count AS count_filtered
      FROM place_action_reports, count_unfiltered, count_filtered
      WHERE
