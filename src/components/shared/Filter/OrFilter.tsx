...
export const OrFilter = memo(({ filterObject, orFilters, orIndex }: Props) => {
  ...
  const onChange = useCallback<InputProps['onChange']>(
    (e, data) => {
      ...
      try {
        // TODO: test
        filterObject.set(newFilter.filter((f) => Object.keys(f).length > 0))
      } catch (error) {
        console.log('OrFilter, error updating app state:', error)
      }
    },
    [filterObject, orFilters, orIndex],
  )
