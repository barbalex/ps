...
import { PlaceCheckReportQuantitiesNode } from './PlaceCheckReportQuantities.tsx'
import { removeChildNodes } from '../../modules/tree/removeChildNodes.ts'
import { addOpenNodes } from '../../modules/tree/addOpenNodes.ts'
import { treeOpenNodesAtom, designingAtom } from '../../store.ts'

export const PlaceCheckReportNode = ({
  projectId,
  subprojectId,
  placeId,
  placeId2,
  ...
  const res = useLiveQuery(
    `SELECT place_check_report_quantities, place_check_report_quantities_in_report
     FROM place_levels
    ...
    <>
      ...
      {isOpen && (
        <>
          ...
          />
          {(isDesigning ||
            placeLevel?.place_check_report_quantities !== false) && (
            <PlaceCheckReportQuantitiesNode
              projectId={projectId}
              subprojectId={subprojectId}
              placeId={placeId}
              placeId2={placeId2}
              placeCheckReportId={nav.id}
              level={level + 1}
            />
          )}
        </>
