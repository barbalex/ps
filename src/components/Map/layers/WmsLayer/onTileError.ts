...
export const onTileError = async (
  ...
  setShortTermOnlineFromFetchError((res as { error?: unknown }).error)
  ...
  const data = xmlToJson(parser.parseFromString(res.data, 'text/html')) as {
    HTML?: {
      BODY?: {
