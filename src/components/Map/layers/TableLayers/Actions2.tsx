...
export const Actions2 = ({ layerPresentation }: { layerPresentation: TableLayerProps['layerPresentation'] }) => {
  ...
  const res = useLiveQuery<Actions>(
    `
    SELECT actions.action_id, actions.place_id, actions.date, actions.data,
  ...
  const actions: Actions[] = res?.rows ?? []
