export const generateUserLabel = async (db) => {
  ...
  // for PostgreSQL:
  const columns = await db.rawQuery({
    sql: `SELECT column_name AS name FROM information_schema.columns WHERE table_name = 'users';`,
  })
  ...
  if (!hasLabel) {
    try {
      ...
      return console.error('error when adding generated label:', error)
    }
    try {
      await db.unsafeExec({
      ...
      return console.error('error when indexing label:', error)
    }
  ...
  // for PostgreSQL:
  const triggers = await db.rawQuery({
    sql: `SELECT trigger_name AS name FROM information_schema.triggers WHERE event_object_table = 'users';`,
  })
  console.log('users, triggers:', triggers)
