export const generatePartialIndexes = async (db) => {
  await db.unsafeExec({
    sql: `
          ...
          CREATE INDEX if not exists ON layer_styles layer_styles_places1_idx USING btree(places1) WHERE places1;
          CREATE INDEX if not exists ON layer_styles layer_styles_places2_idx USING btree(places2) WHERE places2;
          CREATE INDEX if not exists ON layer_styles layer_styles_actions_idx USING btree(actions) WHERE actions;
          CREATE INDEX if not exists ON layer_styles layer_styles_checks_idx USING btree(checks) WHERE checks;
          CREATE INDEX if not exists ON layer_styles layer_styles_observations_idx USING btree(observations) WHERE observations;
          CREATE INDEX if not exists ON layer_styles layer_styles_deleted_idx USING btree(deleted) WHERE deleted;
    `,
  })
