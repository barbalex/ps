...
import type PlaceCheckReportQuantities from '../../models/public/PlaceCheckReportQuantities.ts'
import type Units from '../../models/public/Units.ts'
import type ListValues from '../../models/public/ListValues.ts'

type Props = {
  placeCheckReportQuantityId: string
  projectId: string
}

export const PlaceCheckReportQuantityInline = ({
  ...
  const row: PlaceCheckReportQuantities | undefined = res?.rows?.[0]
  ...
  const onUnitChange = async (_e, data) => {
    ...
    try {
      await db.query(
        `UPDATE place_check_report_quantities SET unit_id = $1 WHERE place_check_report_quantity_id = $2`,
        [value, placeCheckReportQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
      ...
      >
        <RadioGroupField
          ...
          name={`unit_id_${placeCheckReportQuantityId}`}
          list={unitIds}
        ...
        {selectedUnit?.list_id && hasListValues ? (
          listValues.length <= 5 ? (
            <RadioGroupField
              ...
              name={`${unitValueField}_${placeCheckReportQuantityId}`}
              list={listValueIds}
