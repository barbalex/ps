...
import type PlaceCheckReportQuantitiesHistory from '../../models/public/PlaceCheckReportQuantitiesHistory.ts'
import type Units from '../../models/public/Units.ts'
import type ListValues from '../../models/public/ListValues.ts'

export const PlaceCheckReportQuantityHistoryCompare = ({
  from,
}: {
  from:
    ...
    | '/data/projects/$projectId_/subprojects/$subprojectId_/places/$placeId_/places/$placeId2_/check-reports/$placeCheckReportId_/quantities/$placeCheckReportQuantityId_/histories/$placeCheckReportQuantityHistoryId'
}) => {
  ...
  const {
    ...
    placeCheckReportQuantityHistoryId,
  } = useParams({ from, strict: false })
  const placeCheckReportQuantityPath = placeId2
    ? `/data/projects/${projectId}/subprojects/${subprojectId}/places/${placeId}/places/${placeId2}/check-reports/${placeCheckReportId}/quantities/${placeCheckReportQuantityId}`
    : `/data/projects/${projectId}/subprojects/${subprojectId}/places/${placeId}/check-reports/${placeCheckReportId}/quantities/${placeCheckReportQuantityId}`
  const historyPath = `${placeCheckReportQuantityPath}/histories`
  ...
  const onChange = async (e, data) => {
    ...
    try {
      await db.query(
        `UPDATE place_check_report_quantities SET ${name} = $1 WHERE place_check_report_quantity_id = $2`,
        [value, placeCheckReportQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
        ...prev,
  ...
  const onListValueChange = async (valueStr: string | null) => {
    ...
    try {
      await db.query(
        `UPDATE place_check_report_quantities SET ${unitValueField} = $1 WHERE place_check_report_quantity_id = $2`,
        [typedValue, placeCheckReportQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
        ...prev,
  ...
  const formatFieldValue = (
    field: string,
    history: PlaceCheckReportQuantitiesHistory,
  ) => {
    if (field === 'unit_id') {
  ...
  return (
    <HistoryCompare<PlaceCheckReportQuantitiesHistory>
      onBack={() => navigate({ to: placeCheckReportQuantityPath })}
      ...
      row={row}
      historyConfig={{
        historyTable: 'place_check_report_quantities_history',
        rowIdField: 'place_check_report_quantity_id',
        rowId: placeCheckReportQuantityId,
        historyPath,
        routeHistoryId: placeCheckReportQuantityHistoryId,
        currentRow: row,
      }}
      restoreConfig={{
        db,
