...
export const ActionTaxonInline = ({ actionTaxonId, projectId }: Props) => {
  ...
  const row: ActionTaxa | undefined = res?.rows?.[0]
  ...
  const onListValueChange = async (valueStr: string | null) => {
    ...
    try {
      await db.query(
        `UPDATE action_taxa SET ${unitValueField} = $1 WHERE action_taxon_id = $2`,
        [typedValue, actionTaxonId],
      )
    } catch (error) {
      setValidations((prev) => ({
        ...prev,
  ...
  const onUnitChange = async (_e, data) => {
    ...
    try {
      await db.query(
        `UPDATE action_taxa SET unit_id = $1 WHERE action_taxon_id = $2`,
        [value, actionTaxonId],
      )
    } catch (error) {
      setValidations((prev) => ({
        ...
        {selectedUnit?.list_id && hasListValues ? (
          listValues.length <= 5 ? (
            <RadioGroupField
              ...
              name={`${unitValueField}_${actionTaxonId}`}
              list={listValueIds}
