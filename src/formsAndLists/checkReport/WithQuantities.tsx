...
import { CheckReportQuantityInline } from '../checkReportQuantity/Inline.tsx'
...
export const CheckReportWithQuantities = ({ from }) => {
  const { checkReportId, projectId, placeId2 } = useParams({ from })
  ...
  const quantitiesRes = useLiveQuery(
    `SELECT check_report_quantity_id FROM check_report_quantities WHERE check_report_id = $1 ORDER BY check_report_quantity_id`,
    [checkReportId],
  )
  ...
  const addQuantity = async () => {
    await createCheckReportQuantity({ checkReportId })
  }
        ...
        {!res ? (
          ...
          <>
            ...
            {showQuantities && (
              ...
              >
                {quantities.map((q, i) => (
                  <div key={q.check_report_quantity_id}>
                    {i > 0 && (
                      <div
                        style={{
                    ...
                    <CheckReportQuantityInline
                      checkReportQuantityId={q.check_report_quantity_id}
                      projectId={projectId}
                    />
                  </div>
                ))}
