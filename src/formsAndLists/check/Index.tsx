...
export const CheckIndex = ({ from }) => {
  ...
  const res = useLiveQuery(
    `SELECT check_quantities_in_check, check_taxa_in_check FROM place_levels WHERE project_id = $1 AND level = $2`,
  ...
  return quantitiesInCheck || taxaInCheck ? (
    <CheckWithAll from={from} />
  ) : (
    <Check from={from} />
  )
}
