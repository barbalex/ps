...
import type ActionQuantities from '../../models/public/ActionQuantities.ts'
...
export const ActionQuantityInline = ({
  actionQuantityId,
  projectId,
}: Props) => {
  ...
  const row: ActionQuantities | undefined = res?.rows?.[0]
  ...
  const onChange = async (e, data) => {
    ...
    try {
      await db.query(
        `UPDATE action_quantities SET ${name} = $1 WHERE action_quantity_id = $2`,
        [value, actionQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
        ...prev,
  ...
  const onUnitChange = async (_e, data) => {
    ...
    try {
      await db.query(
        `UPDATE action_quantities SET unit_id = $1 WHERE action_quantity_id = $2`,
        [value, actionQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
  ...
  const onListValueChange = async (valueStr: string | null) => {
    ...
    try {
      await db.query(
        `UPDATE action_quantities SET ${unitValueField} = $1 WHERE action_quantity_id = $2`,
        [typedValue, actionQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
        ...prev,
  ...
  const deleteRow = async () => {
    try {
      await db.query(
        'DELETE FROM action_quantities WHERE action_quantity_id = $1',
        [actionQuantityId],
      )
      addOperation({
        table: 'action_quantities',
      ...
      console.error('Error deleting action quantity:', error)
    }
  }
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
      ...
      >
        <RadioGroupField
          ...
          name={`unit_id_${actionQuantityId}`}
          list={unitIds}
        ...
        {selectedUnit?.list_id && hasListValues ? (
          listValues.length <= 5 ? (
            <RadioGroupField
              ...
              name={`${unitValueField}_${actionQuantityId}`}
              list={listValueIds}
