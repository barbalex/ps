...
import type ActionReportQuantities from '../../models/public/ActionReportQuantities.ts'
...
export const ActionReportQuantityInline = ({
  ...
  const row: ActionReportQuantities | undefined = res?.rows?.[0]
  ...
  const onChange = async (e, data) => {
    ...
    try {
      await db.query(
        `UPDATE action_report_quantities SET ${name} = $1 WHERE action_report_quantity_id = $2`,
        [value, actionReportQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
        ...prev,
  ...
  const onUnitChange = async (_e, data) => {
    ...
    try {
      await db.query(
        `UPDATE action_report_quantities SET unit_id = $1 WHERE action_report_quantity_id = $2`,
        [value, actionReportQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
  ...
  const onListValueChange = async (valueStr: string | null) => {
    ...
    try {
      await db.query(
        `UPDATE action_report_quantities SET ${unitValueField} = $1 WHERE action_report_quantity_id = $2`,
        [typedValue, actionReportQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
        ...prev,
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
      ...
      >
        <RadioGroupField
          ...
          name={`unit_id_${actionReportQuantityId}`}
          list={unitIds}
        ...
        {selectedUnit?.list_id && hasListValues ? (
          listValues.length <= 5 ? (
            <RadioGroupField
              ...
              name={`${unitValueField}_${actionReportQuantityId}`}
              list={listValueIds}
