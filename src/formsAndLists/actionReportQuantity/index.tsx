...
export const ActionReportQuantity = ({ from }) => {
  ...
  const onChange = async (e, data) => {
    ...
    try {
      await db.query(
        `UPDATE action_report_quantities SET ${name} = $1 WHERE place_action_report_quantity_id = $2`,
  ...
  const onListValueChange = async (valueStr: string | null) => {
    ...
    try {
      await db.query(
        `UPDATE action_report_quantities SET ${unitValueField} = $1 WHERE place_action_report_quantity_id = $2`,
