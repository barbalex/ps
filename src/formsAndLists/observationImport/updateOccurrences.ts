...
const calculateGeometry = (occurrenceData, observationImport, proj4Setup) => {
  if (
  ...
  const coordinates = [
    occurrenceData[observationImport.x_coordinate_field],
    occurrenceData[observationImport.y_coordinate_field],
  ]
 ...
 */
export const replaceOccurrences = async ({ file, observationImport, db }) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async () => {
      try {
        ...
        await db.query(
          `DELETE FROM occurrences WHERE observation_import_id = $1`,
        ...
        for (const occ of occurrences) {
          ...
          await db.query(
            `INSERT INTO occurrences (observation_import_id, account_id, occurrence_id, data, id_in_source, geometry, label) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
            [
    ...
    reader.onload = async () => {
      try {
        ...
        const existingResult = await db.query(
          `SELECT occurrence_id, data, place_id, not_to_assign, comment FROM occurrences WHERE observation_import_id = $1`,
          [observationImport.observation_import_id],
        )
