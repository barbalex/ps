...
export const Header = ({
  ...
  // Keep a ref to the current observationImportId so it's always fresh in callbacks
  // without this users can only click toNext or toPrevious once
  const observationImportIdRef = useRef(observationImportId)
  useEffect(() => {
    observationImportIdRef.current = observationImportId
  }, [observationImportId])
  ...
  const deleteRow = async () => {
    try {
      await db.query(
        `DELETE FROM observation_imports WHERE observation_import_id = $1`,
        [observationImportId],
      )
      addOperation({
  ...
  const toNext = async () => {
    try {
      ...
      navigate({
        to: `../${next.observation_import_id}`,
        params: (prev) => ({
          ...prev,
          observationImportId: next.observation_import_id,
        }),
      })
    } catch (error) {
      console.error(error)
  ...
  const toPrevious = async () => {
    try {
      ...
      navigate({
        to: `../${previous.observation_import_id}`,
        params: (prev) => ({
          ...prev,
          observationImportId: previous.observation_import_id,
        }),
      })
    } catch (error) {
      console.error(error)
