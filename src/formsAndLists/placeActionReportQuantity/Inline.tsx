...
type Props = {
  placeActionReportQuantityId: string
  projectId: string
}

export const PlaceActionReportQuantityInline = ({
  ...
  const onUnitChange = async (_e, data) => {
    ...
    try {
      await db.query(
        `UPDATE place_action_report_quantities SET unit_id = $1 WHERE place_action_report_quantity_id = $2`,
        [value, placeActionReportQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
      ...
      >
        <RadioGroupField
          ...
          name={`unit_id_${placeActionReportQuantityId}`}
          list={unitIds}
        ...
        {selectedUnit?.list_id && hasListValues ? (
          listValues.length <= 5 ? (
            <RadioGroupField
              ...
              name={`${unitValueField}_${placeActionReportQuantityId}`}
              list={listValueIds}
