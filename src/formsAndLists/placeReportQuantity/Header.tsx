...
export const Header = ({ autoFocusRef, from }) => {
  const { placeReportId, placeReportQuantityId } = useParams({ from })
  ...
  const placeReportQuantityIdRef = useRef(placeReportQuantityId)
  useEffect(() => {
    placeReportQuantityIdRef.current = placeReportQuantityId
  }, [placeReportQuantityId])

  const countRes = useLiveQuery(
    `SELECT COUNT(*) as count FROM place_report_quantities WHERE place_report_id = '${placeReportId}'`,
  )
  ...
  const toNext = async () => {
    try {
      ...
      const index = quantities.findIndex(
        (p) => p.place_report_quantity_id === placeReportQuantityIdRef.current,
      )
      const next = quantities[(index + 1) % len]
      navigate({
        to: `../${next.place_report_quantity_id}`,
        params: (prev) => ({
          ...prev,
          placeReportQuantityId: next.place_report_quantity_id,
        }),
      })
    } catch (error) {
  ...
  const toPrevious = async () => {
    try {
      ...
      navigate({
        to: `../${previous.place_report_quantity_id}`,
        params: (prev) => ({
          ...prev,
          placeReportQuantityId: previous.place_report_quantity_id,
        }),
      })
    } catch (error) {
      console.error(
