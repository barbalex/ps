...
import type CheckReportQuantities from '../../models/public/CheckReportQuantities.ts'
...
export const CheckReportQuantityInline = ({
  ...
  const onChange = async (e, data) => {
    ...
    try {
      await db.query(
        `UPDATE check_report_quantities SET ${name} = $1 WHERE check_report_quantity_id = $2`,
  ...
  const onUnitChange = async (_e, data) => {
    ...
    try {
      await db.query(
        `UPDATE check_report_quantities SET unit_id = $1 WHERE check_report_quantity_id = $2`,
        [value, checkReportQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
  ...
  const onListValueChange = async (valueStr: string | null) => {
    ...
    try {
      await db.query(
        `UPDATE check_report_quantities SET ${unitValueField} = $1 WHERE check_report_quantity_id = $2`,
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
      ...
      >
        <RadioGroupField
          ...
          name={`unit_id_${checkReportQuantityId}`}
          list={unitIds}
        ...
        {selectedUnit?.list_id && hasListValues ? (
          listValues.length <= 5 ? (
            <RadioGroupField
              ...
              name={`${unitValueField}_${checkReportQuantityId}`}
              list={listValueIds}
