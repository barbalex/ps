...
import type CheckQuantities from '../../models/public/CheckQuantities.ts'
...
export const CheckQuantityInline = ({ checkQuantityId, projectId }: Props) => {
  ...
  const onUnitChange = async (_e, data) => {
    ...
    try {
      await db.query(
        `UPDATE check_quantities SET unit_id = $1 WHERE check_quantity_id = $2`,
        [value, checkQuantityId],
      )
    } catch (error) {
      setValidations((prev) => ({
  ...
  const deleteRow = async () => {
    try {
      ...
      console.error('Error deleting check quantity:', error)
    }
  }
        ...
        {selectedUnit?.list_id && hasListValues ? (
          listValues.length <= 5 ? (
            <RadioGroupField
              ...
              name={`${unitValueField}_${checkQuantityId}`}
              list={listValueIds}
