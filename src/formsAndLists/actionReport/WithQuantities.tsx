...
import { ActionReportQuantityInline } from '../actionReportQuantity/Inline.tsx'
...
export const ActionReportWithQuantities = ({ from }) => {
  const { actionReportId, projectId, placeId2 } = useParams({ from })
  ...
  const quantitiesRes = useLiveQuery(
    `SELECT action_report_quantity_id FROM action_report_quantities WHERE action_report_id = $1 ORDER BY action_report_quantity_id`,
    [actionReportId],
  )
  ...
  const addQuantity = async () => {
    await createActionReportQuantity({ actionReportId })
  }
        ...
        {!res ? (
          ...
          <>
            ...
            {showQuantities && (
              ...
              >
                {quantities.map((q, i) => (
                  <div key={q.action_report_quantity_id}>
                    {i > 0 && (
                      <div
                        style={{
                    ...
                    <ActionReportQuantityInline
                      actionReportQuantityId={q.action_report_quantity_id}
                      projectId={projectId}
                    />
                  </div>
                ))}
          ...
          <NotFound
            table={formatMessage({ id: 'Z8jucQ', defaultMessage: 'Bericht' })}
            id={actionReportId}
          />
        )}
      </div>
