...
CREATE TABLE IF NOT EXISTS auth_sessions(
  auth_session_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS auth_accounts(
  auth_account_id uuid PRIMARY KEY DEFAULT uuidv7(),
  id uuid DEFAULT NULL,
...
COMMENT ON TABLE auth_accounts IS 'Goal: earn money. Separate from users to allow for multiple auth_accounts per user. Enables seeing the account history.';
COMMENT ON COLUMN auth_accounts.user_id IS 'user that owns the account. null for auth_accounts that are not owned by a user';
COMMENT ON COLUMN auth_accounts.sso_account_id IS 'The ID of the account as provided by the SSO or equal to userId for credential auth_accounts';
COMMENT ON COLUMN auth_accounts.provider_id IS 'The ID of the provider';
COMMENT ON COLUMN auth_accounts.access_token IS 'The access token for the account. Returned by the provider';
COMMENT ON COLUMN auth_accounts.refresh_token IS 'The refresh token for the account';
COMMENT ON COLUMN auth_accounts.access_token_expires_at IS 'The time when the access token expires';
COMMENT ON COLUMN auth_accounts.refresh_token_expires_at IS 'The time when the refresh token expires';
COMMENT ON COLUMN auth_accounts.scope IS 'The scope of the account. Returned by the provider';
COMMENT ON COLUMN auth_accounts.id_token IS 'The ID token for the account. Returned by the provider';
COMMENT ON COLUMN auth_accounts.password IS 'The password of the account. Mainly used for email and password authentication';
...
CREATE INDEX IF NOT EXISTS accounts_period_end_idx ON accounts USING btree(period_end);
...
CREATE TABLE IF NOT EXISTS auth_verifications(
  auth_verification_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS projects(
  ...
  actions_default_unit_id uuid DEFAULT NULL, -- FK to units added below after units table
  ...
  enable_histories boolean DEFAULT TRUE,
...
CREATE INDEX IF NOT EXISTS projects_actions_default_unit_id_idx ON projects USING btree(actions_default_unit_id);
...
CREATE INDEX IF NOT EXISTS projects_place_action_reports_default_unit_id_idx ON projects USING btree(action_reports_default_unit_id);
...
COMMENT ON COLUMN projects.actions_default_unit_id IS 'Default unit for action values.';
...
COMMENT ON COLUMN projects.check_reports_default_unit_id IS 'Default unit for place check report quantities. Can be overwritten in check_reports';
COMMENT ON COLUMN projects.action_reports_default_unit_id IS 'Default unit for place action report quantities. Can be overwritten in action_reports';
...
COMMENT ON COLUMN projects.enable_histories IS 'Whether history comparison is enabled for places. Preset: true.';
...
CREATE TABLE IF NOT EXISTS place_levels(
  place_level_id uuid PRIMARY KEY DEFAULT uuidv7(),
  ...
  action_files_in_action boolean DEFAULT TRUE,
  check_files boolean DEFAULT TRUE,
  check_files_in_check boolean DEFAULT TRUE,
...
COMMENT ON COLUMN place_levels.check_reports IS 'Are check reports for places used? Preset: true';
COMMENT ON COLUMN place_levels.check_report_quantities IS 'Are place check report quantities used? Preset: true';
COMMENT ON COLUMN place_levels.check_report_quantities_in_report IS 'Show place check report quantities inside the report form instead of a separate route? Preset: true';
COMMENT ON COLUMN place_levels.action_reports IS 'Are action reports for places used? Preset: true';
COMMENT ON COLUMN place_levels.action_report_quantities IS 'Are place action report quantities used? Preset: true';
COMMENT ON COLUMN place_levels.action_report_quantities_in_report IS 'Show place action report quantities inside the report form instead of a separate route? Preset: true';
...
COMMENT ON COLUMN place_levels.action_files_in_action IS 'Render files inside the action form? Preset: true';
COMMENT ON COLUMN place_levels.check_files IS 'Are files used for checks on this level? Preset: false';
COMMENT ON COLUMN place_levels.check_files_in_check IS 'Render files inside the check form? Preset: true';
...
CREATE TABLE IF NOT EXISTS subprojects(
  subproject_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  name text DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS taxonomies(
  taxonomy_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE IF NOT EXISTS taxa(
  taxon_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS subproject_taxa(
  subproject_taxon_id uuid PRIMARY KEY DEFAULT uuidv7(),
  subproject_id uuid DEFAULT NULL REFERENCES subprojects(subproject_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE IF NOT EXISTS list_values(
  list_value_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
ALTER TABLE projects ADD CONSTRAINT projects_actions_default_unit_id_fkey FOREIGN KEY (actions_default_unit_id) REFERENCES units(unit_id) ON DELETE SET NULL ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED;
...
ALTER TABLE projects ADD CONSTRAINT projects_place_action_reports_default_unit_id_fkey FOREIGN KEY (action_reports_default_unit_id) REFERENCES units(unit_id) ON DELETE SET NULL ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED;
...
CREATE TABLE IF NOT EXISTS action_quantities(
  action_quantity_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS action_taxa(
  action_taxon_id uuid PRIMARY KEY DEFAULT uuidv7(),
  action_id uuid DEFAULT NULL REFERENCES actions(action_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  taxon_id uuid DEFAULT NULL REFERENCES taxa(taxon_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  unit_id uuid DEFAULT NULL REFERENCES units(unit_id) ON DELETE NO action ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  quantity_integer integer DEFAULT NULL,
...
CREATE INDEX IF NOT EXISTS action_taxa_quantity_integer_idx ON action_taxa USING btree(quantity_integer);
CREATE INDEX IF NOT EXISTS action_taxa_quantity_numeric_idx ON action_taxa USING btree(quantity_numeric);
...
CREATE TABLE IF NOT EXISTS checks(
  check_id uuid PRIMARY KEY DEFAULT uuidv7(),
  place_id uuid DEFAULT NULL REFERENCES places(place_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  date date DEFAULT CURRENT_DATE,
  data jsonb DEFAULT NULL,
  geometry geometry(GeometryCollection, 4326) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS check_quantities(
  check_quantity_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS check_taxa(
  check_taxon_id uuid PRIMARY KEY DEFAULT uuidv7(),
  check_id uuid DEFAULT NULL REFERENCES checks(check_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  taxon_id uuid DEFAULT NULL REFERENCES taxa(taxon_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  unit_id uuid DEFAULT NULL REFERENCES units(unit_id) ON DELETE NO action ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  quantity_integer integer DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS check_reports(
  place_check_report_id uuid PRIMARY KEY DEFAULT uuidv7(),
  place_id uuid DEFAULT NULL REFERENCES places(place_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE IF NOT EXISTS check_report_quantities(
  place_check_report_quantity_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS action_reports(
  place_action_report_id uuid PRIMARY KEY DEFAULT uuidv7(),
  place_id uuid DEFAULT NULL REFERENCES places(place_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  year integer DEFAULT DATE_PART('year', now()::date),
  data jsonb DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS action_report_quantities(
  place_action_report_quantity_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE INDEX IF NOT EXISTS place_action_report_quantities_quantity_numeric_idx ON action_report_quantities USING btree(quantity_numeric);
CREATE INDEX IF NOT EXISTS place_action_report_quantities_quantity_text_idx ON action_report_quantities USING btree(quantity_text);
...
CREATE TABLE IF NOT EXISTS user_messages(
  user_message_id uuid PRIMARY KEY DEFAULT uuidv7(),
  user_id uuid DEFAULT NULL REFERENCES users(user_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE IF NOT EXISTS goal_reports(
  goal_report_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS subproject_report_designs(
  subproject_report_design_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  name text DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS project_reports(
  project_report_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  year integer DEFAULT DATE_PART('year', now()::date),
  data jsonb DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS project_report_subdesigns(
  project_report_subdesign_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  name text DEFAULT NULL,
  design jsonb DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS project_report_designs(
  project_report_design_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  name text DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS field_types(
  field_type_id uuid PRIMARY KEY DEFAULT uuidv7(),
  name text DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS widget_types(
  widget_type_id uuid PRIMARY KEY DEFAULT uuidv7(),
  name text DEFAULT NULL,
  -- no account_id as field_types are predefined for all projects
...
CREATE TABLE IF NOT EXISTS widgets_for_fields(
  widget_for_field_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS field_sorts(
  field_sort_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  table_name text DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS observation_imports(
  observation_import_id uuid PRIMARY KEY DEFAULT uuidv7(),
  subproject_id uuid DEFAULT NULL REFERENCES subprojects(subproject_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE IF NOT EXISTS wms_services(
  wms_service_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS wms_service_layers(
  wms_service_layer_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS wms_layers(
  wms_layer_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid NOT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  wms_service_id uuid DEFAULT NULL REFERENCES wms_services(wms_service_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE IF NOT EXISTS wfs_services(
  wfs_service_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid NOT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  url text DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS wfs_service_layers(
  wfs_service_layer_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS vector_layer_geoms(
  vector_layer_geom_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS vector_layer_displays(
  vector_layer_display_id uuid PRIMARY KEY DEFAULT uuidv7(),
  vector_layer_id uuid DEFAULT NULL REFERENCES vector_layers(vector_layer_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE IF NOT EXISTS charts(
  chart_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  subproject_id uuid DEFAULT NULL REFERENCES subprojects(subproject_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  place_id uuid DEFAULT NULL REFERENCES places(place_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE IF NOT EXISTS chart_subjects(
  chart_subject_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS crs(
  crs_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS project_crs(
  project_crs_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
--
CREATE TYPE qcs_level_enum AS ENUM ('root', 'project', 'subproject');
CREATE TYPE project_qcs_level_enum AS ENUM ('project', 'subproject');

CREATE TABLE IF NOT EXISTS qcs(
  qcs_id uuid PRIMARY KEY DEFAULT uuidv7(),
...
CREATE TABLE IF NOT EXISTS qc_assignments(
  qc_assignment_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  subproject_id uuid DEFAULT NULL REFERENCES subprojects(subproject_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE INDEX IF NOT EXISTS qc_assignments_project_id_idx ON qc_assignments USING btree(project_id);
CREATE INDEX IF NOT EXISTS qc_assignments_subproject_id_idx ON qc_assignments USING btree(subproject_id);
CREATE INDEX IF NOT EXISTS qc_assignments_qc_id_idx ON qc_assignments USING btree(qc_id);
CREATE INDEX IF NOT EXISTS qc_assignments_label_idx ON qc_assignments USING btree(label);

COMMENT ON TABLE qc_assignments IS 'Quality controls assigned to a project or subproject. The level(s) of applicability (root, project, subproject) are defined on the qcs table itself.';

--------------------------------------------------------------
-- project_qcs: project-specific quality controls
--
CREATE TABLE IF NOT EXISTS project_qcs(
  project_qc_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid NOT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  name_de text DEFAULT NULL,
  name_en text DEFAULT NULL,
  name_fr text DEFAULT NULL,
...
--------------------------------------------------------------
-- project_qc_assignments
--
CREATE TABLE IF NOT EXISTS project_qc_assignments(
  project_qc_assignment_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  subproject_id uuid DEFAULT NULL REFERENCES subprojects(subproject_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE INDEX IF NOT EXISTS project_qc_assignments_project_id_idx ON project_qc_assignments USING btree(project_id);
CREATE INDEX IF NOT EXISTS project_qc_assignments_subproject_id_idx ON project_qc_assignments USING btree(subproject_id);
CREATE INDEX IF NOT EXISTS project_qc_assignments_project_qc_id_idx ON project_qc_assignments USING btree(project_qc_id);
CREATE INDEX IF NOT EXISTS project_qc_assignments_label_idx ON project_qc_assignments USING btree(label);
...
CREATE TABLE IF NOT EXISTS exports(
  exports_id uuid PRIMARY KEY DEFAULT uuidv7(),
  name_de text DEFAULT NULL,
  name_en text DEFAULT NULL,
  name_fr text DEFAULT NULL,
  name_it text DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS export_assignments(
  export_assignment_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  subproject_id uuid DEFAULT NULL REFERENCES subprojects(subproject_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE IF NOT EXISTS project_exports(
  project_exports_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid NOT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  name_de text DEFAULT NULL,
  name_en text DEFAULT NULL,
  name_fr text DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS project_export_assignments(
  project_export_assignment_id uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid DEFAULT NULL REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  subproject_id uuid DEFAULT NULL REFERENCES subprojects(subproject_id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
