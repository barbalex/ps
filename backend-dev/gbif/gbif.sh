# works
curl -X 'GET' \
  'https://api.gbif.org/v1/observation/search?country=CH&q=rosa&limit=3&offset=0' \
...
curl -X 'GET' \
  'https://api.gbif.org/v1/observation/search?datasetKey=83fdfd3d-3a25-4705-9fbe-3db1d1892b13&limit=3&offset=0' \
  -H 'accept: application/json'
  
curl -X 'GET' \
  'https://api.gbif.org/v1/observation/search?datasetKey=83fdfd3d-3a25-4705-9fbe-3db1d1892b13&scientificName=pulsatilla%vulgaris&limit=3&offset=0' \
  -H 'accept: application/json'

# search by species key
curl -X 'GET' \
  'https://api.gbif.org/v1/observation/search?datasetKey=83fdfd3d-3a25-4705-9fbe-3db1d1892b13&speciesKey=3172062&limit=3&offset=0' \
  -H 'accept: application/json'

# works: search by taxonID (url-encode the taxonID: infospecies.ch:infoflora:59100)
curl -X 'GET' \
  'https://api.gbif.org/v1/observation/search?datasetKey=83fdfd3d-3a25-4705-9fbe-3db1d1892b13&taxonID=infospecies.ch%3Ainfoflora%3A59100&limit=3&offset=0' \
  -H 'accept: application/json'
# BUT: how to get a list of speciesKeys for a dataset?

curl -X 'GET' \
  'https://api.gbif.org/v1/observation/search?speciesKey=3172062&&limit=3&offset=0' \
  -H 'accept: application/json'

  # species download from the observation dataset: GBIF.org (24 January 2024) GBIF Observation Download  https://doi.org/10.15468/dl.ctsxky
